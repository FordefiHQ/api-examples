...
| `depositType` | string | Where you'll deposit from. `ORIGIN_CHAIN` = deposit on source blockchain. `INTENTS` = deposit from intents.near balance |
| `recipientType` | string | Where output goes. `DESTINATION_CHAIN` = send to destination blockchain. `INTENTS` = credit to intents.near balance |
| `refundType` | string | Where refunds go. `ORIGIN_CHAIN` = refund to source blockchain. `INTENTS` = refund to intents.near balance |
| `deadline` | string | ISO 8601 timestamp. Quote expires after this. User must deposit before deadline. Typically set ~10 min from now |
...
// committed.quote.deadline = "2026-01-16T15:00:00Z" → must deposit before
```
