...
3. **Poll status** by `depositAddress` until terminal: `SUCCESS`, `FAILED`, `REFUNDED`, `INCOMPLETE_DEPOSIT`
4. **Required quote fields** - `depositType`, `recipientType`, `refundType`, and `deadline` (ISO 8601) are all required in POST /v0/quote
5. **Auth header** - use `Authorization: Bearer <key>`, not `x-api-key`
...
2. **API Reference (CRITICAL)**
   ...
   - [api-status](rules/api-status.md) - Poll by depositAddress until terminal state (SUCCESS, FAILED, REFUNDED)
