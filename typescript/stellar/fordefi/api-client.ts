...
import {
  FordefiStellarConfig,
  CreateStellarRequest,
...
export async function createTransaction(
  config: FordefiStellarConfig,
  request: CreateStellarRequest
): Promise<StellarTransactionResponse> {
  const path = TRANSACTIONS_API_PATH;
  const url = `${FORDEFI_API_BASE_URL}${path}`;
  const timestamp = currentTimestampSeconds();
  const requestBody = JSON.stringify(request);

  console.log(`Submitting Stellar ${request.type} via Fordefi...`);

  const response = await axios.post<StellarTransactionResponse>(url, requestBody, {
...
export async function getTransactionStatus(
  ...
  const response = await axios.get<StellarTransactionResponse>(url, {
