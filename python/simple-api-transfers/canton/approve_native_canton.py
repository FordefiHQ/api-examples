...
async def build_request(vault_id: str, custom_note: str):
    request_json = {
        ...
        "details": {
            "type": CantonTransactionDetailType.CANTON_PRE_APPROVAL_SETUP.value,
            "asset_identifier": {
...
custom_note = "Approving native CC transfers" # Optional note

async def main():
    try:
        ## Building pre-approval setup transaction (auto-accepts incoming native CC transfers)
        ...
        print(f"✅ Pre-approval setup submitted successfully! Transaction ID: {resp.json()['id']}")
    except Exception as e:
        print(f"❌ Pre-approval setup failed: {str(e)}")
